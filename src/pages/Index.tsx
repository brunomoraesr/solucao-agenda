...
import { Calendar, Clock, User, Mail, Phone, CheckCircle2, Users, ArrowLeft, Hash, CalendarCheck } from "lucide-react";
...
const generateTimeSlots = (period: "morning" | "afternoon", day: DaySchedule, sessionType: SessionType | null, bookings: Booking[]): TimeSlot[] => {
  ...
  if (sessionType === "pilates" && day.pilatesOnlyAt11) {
    ...
    const availableFor: Professional[] = professionals.filter(prof => {
      return !bookings.some(booking => booking.day === day.day && booking.period === period && booking.time === timeString && booking.professional === prof);
    });
    return [{
      time: timeString,
      availableFor
...
const Index = () => {
  ...
  const getAvailableSessionTypes = (): SessionType[] => {
    ...
    if (selectedDay.day === "Terça-feira") {
      return selectedDay.periods.map(p => p === "morning" ? "pilates" : "massagem") as SessionType[];
    }
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-secondary/30 to-accent/10">
      <div className="container mx-auto px-4 py-12 relative">
        {step > 1 && <Button variant="ghost" size="icon" onClick={handleBack} className="absolute top-8 left-4 md:top-12 md:left-8 w-12 h-12 z-10
                       bg-card/80 border border-border hover:bg-card shadow-md
                       transition-all duration-300 rounded-full" aria-label="Voltar à etapa anterior">
        ...
        <div className="text-center mb-12 animate-in fade-in duration-700">
          ...
          <div className="mt-6">
            <Button onClick={() => setShowBookings(!showBookings)} variant="outline" className="gap-2">
        ...
        {/* Progress Steps */}
        {!showBookings && <div className="max-w-4xl mx-auto mb-12">
          <div className="flex items-start justify-center px-2">
            ...
            return <Fragment key={s}>
                ...
                width: "4.5rem"
                      ...
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-300 ${step >= s ? "bg-gradient-to-br from-primary to-accent text-primary-foreground shadow-elegant scale-110" : "bg-muted text-muted-foreground"}`}>
                        {s}
                      </div>
                    ...
                    {s < 5 && <div className={`flex-1 h-1 rounded-full transition-all duration-300 mt-4 ${step > s ? "bg-gradient-to-r from-primary to-accent" : "bg-muted"}`} style={{
                minWidth: "2rem"
              }} />}
                  </Fragment>;
          })}
        ...
        {!showBookings && step === 1 && <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom duration-500">
            <Card className="p-8 shadow-soft border-border/50 backdrop-blur-sm bg-card/80">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {availableSchedule.map(day => <button key={day.day} onClick={() => handleDaySelect(day)} className="p-6 rounded-xl border-2 border-border hover:border-primary bg-card hover:bg-primary/5 transition-all duration-300 text-left group hover:shadow-elegant hover:scale-105">
                    <div className="font-semibold text-lg mb-2 group-hover:text-primary transition-colors">
                      {day.day}
                    </div>
                    <div className="flex gap-2 flex-wrap mb-2">
                      {day.periods.map(period => <span key={period} className="px-2 py-1 rounded-full text-xs font-medium bg-primary/10 text-primary border border-primary/20">
        ...
        {!showBookings && step === 2 && selectedDay && <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom duration-500">
            <Card className="p-8 shadow-soft border-border/50 backdrop-blur-sm bg-card/80">
              <div className="mb-6">
                <Button variant="ghost" onClick={handleBack} className="mb-4 hover:bg-primary/10">
                  ← Voltar
                </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {getAvailableSessionTypes().map(sessionType => <Button key={sessionType} onClick={() => handleSessionTypeSelect(sessionType)} variant="outline"
        ...
        {!showBookings && step === 3 && selectedDay && selectedSessionType && <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom duration-500">
            <Card className="p-8 shadow-soft border-border/50 backdrop-blur-sm bg-card/80">
              <div className="mb-6">
                <Button variant="ghost" onClick={handleBack} className="mb-4 hover:bg-primary/10">
                  ← Voltar
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {professionals.map(professional => <Button key={professional} onClick={() => handleProfessionalSelect(professional)} variant="outline"
            // === AJUSTE DE COR DO TEXTO NO HOVER ===
        ...
        {step === 4 && selectedDay && selectedProfessional && <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom duration-500">
            <Card className="p-8 shadow-soft border-border/50 backdrop-blur-sm bg-card/80">
              <div className="mb-6">
                <Button variant="ghost" onClick={handleBack} className="mb-4 hover:bg-primary/10">
                  ← Voltar
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-3">
                  {getAvailablePeriodsForSession().map(period => <Button key={period} onClick={() => handlePeriodSelect(period)} variant={selectedPeriod === period ? "default" : "outline"} className={`flex-1 h-14 text-base transition-all duration-300 ${selectedPeriod === period ? "shadow-elegant scale-105" :
              // === AJUSTE DE COR DO TEXTO NO HOVER ===
              ...
              {/* Time Slots */}
              {selectedPeriod && <div className="animate-in fade-in duration-500">
                ...
                return <Button key={slot.time} onClick={() => handleTimeSelect(slot.time)} variant={selectedTime === slot.time ? "default" : "outline"} disabled={!isAvailable} className={`h-14 text-base font-semibold transition-all duration-300 ${selectedTime === slot.time ? "shadow-elegant scale-105" : isAvailable ?
                // === AJUSTE DE COR DO TEXTO NO HOVER (E BORDA) ===
                "hover:border-primary hover:bg-primary/5 hover:text-primary" : "opacity-50 cursor-not-allowed"}`}>
                          {slot.time}
                        </Button>;
              })}
                  </div>
                  {timeSlots.every(slot => !slot.availableFor.includes(selectedProfessional!)) && <p className="text-center text-muted-foreground mt-4">
        ...
        {!showBookings && step === 5 && <div className="max-w-2xl mx-auto animate-in slide-in-from-bottom duration-500">
            <Card className="p-8 shadow-soft border-border/50 backdrop-blur-sm bg-card/80">
              <div className="mb-6">
                <Button variant="ghost" onClick={handleBack} className="mb-4 hover:bg-primary/10">
                  ← Voltar
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  ...formData,
                  name: e.target.value
                })} className="pl-11 h-12 border-border focus:border-primary transition-colors" placeholder="Seu nome" required />
                  </div>
                </div>
                ...
                })} className="pl-11 h-12 border-border focus:border-primary transition-colors" placeholder="seu@email.com" required />
                  </div>
                </div>
                ...
                })} className="pl-11 h-12 border-border focus:border-primary transition-colors" placeholder="(00) 00000-0000" required />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input id="ramal" type="tel" // 'tel' é bom para abrir teclado numérico em mobile
                value={formData.ramal} onChange={e => setFormData({
                  ...formData,
                  // Garante que apenas números sejam inseridos
                  ramal: e.target.value.replace(/\D/g, "")
                })} className="pl-11 h-12 border-border focus:border-primary transition-colors" placeholder="0000" maxLength={4} // Limita a 4 dígitos
                ...
                <Button type="submit" className="w-full h-14 text-lg font-semibold shadow-elegant hover:scale-105 transition-all duration-300 bg-gradient-to-r from-primary to-accent">
